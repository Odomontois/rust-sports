...
#[derive(Debug, Clone)]
struct RangeCalc<A, M> { measures: Vec<M>, values: Vec<A>, add: Option<Vec<M>>, cap: usize }

impl<A, M> RangeCalc<A, M> {
    fn new() -> Self {
        RangeCalc { values: Vec::new(), measures: Vec::new(), add: None, cap: 1 }
    }
}
...
impl<A, M: Monoid> RangeCalc<A, M> where A: Measured<M> {
...
impl<A: Measured<M>, M: Rig> RangeCalc<A, M> {
...
impl<'a, A, M> IntoIterator for &'a RangeCalc<A, M> {
    type Item = &'a A;
...
impl<A: Measured<M>, M: Monoid + Clone> FromIterator<A> for RangeCalc<A, M> {
    fn from_iter<T: IntoIterator<Item=A>>(iter: T) -> Self {
        let mut res = RangeCalc::new();
        for x in iter { res.push(x) }
        res
    }
}

impl<A: Measured<M>, M: Monoid + Clone> Measured<M> for RangeCalc<A, M> {
    fn measure(&self) -> M {
...
mod test {
    ...
    #[test]
    fn test_fen_small() {
        let mut f: RangeCalc<i64, i64> = RangeCalc::new();
        for i in 1..100i64 {
            println!("{:?} ", f);
            f.push(i);
        }
    ...
    fn test_fen() {
        ...
        let f: RangeCalc<i64, i64> = (0..=size).collect();
        for _ in 1..count {
